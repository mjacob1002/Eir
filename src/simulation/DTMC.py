...
# tested
def dtmc_sir(beta: float, mu_ir: float, S0: int, I0: int, R0: int, time: int, xlabel: str, plot = True):
    """
    ...
    :param plot: if true, the method will display the plot. Otherwise, it just returns the arrays
    :return: S, I , R, t arrays
    """
    n = S0 + I0 + R0
    ...
    for i in range(1, time + 1):
        p = beta * I[i - 1] / n
        print("P: ", p, " I: ", I[i - 1])
        s_i = np.random.binomial(n=int(S[i - 1]), p=p, size=1000).mean()
        i_r = np.random.binomial(n=int(I[i - 1]), p=mu_ir, size=1000).mean()
        S[i] = S[i - 1] - s_i
        I[i] = I[i - 1] + s_i - i_r
        R[i] = R[i - 1] + i_r
    if plot == True:
...
# tested
def dtmc_sirv(t: int, beta: float, mu_ir: float, mu_sv: float, S0: int, I0: int, R0: int, V0 = 0.0, x_label = "Days", plot = True):
    """
    ...
    :param plot: if true, display the plot of all arrays. Otherwise it will just return arrays
    ...
    for i in range(1, t + 1):
        p = beta * I[i - 1] / n
        s_i = np.random.binomial(n=S[i - 1], p=p, size=1000).mean()
        i_r = np.random.binomial(n=I[i - 1], p=mu_ir, size=1000).mean()
        Stemp = int(S[i - 1] - s_i)
        s_v = np.random.binomial(n=Stemp, p=mu_sv, size=1000).mean()
        S[i] = S[i - 1] - s_i - s_v
        I[i] = I[i - 1] + s_i - i_r
        R[i] = R[i - 1] + i_r
        V[i] = V[i - 1] + s_v
...
# tested
def dtmc_sis(beta: float, mu_is: float, S0: int, I0: int, t: int, x_label: str, plot = True):
    """
    ...
    for i in range(1, t + 1):
        p = beta * I[i - 1] / n
        s_i = np.random.binomial(S[i - 1], p, 1000).mean()
        i_s = np.random.binomial(I[i - 1], mu_is, 1000).mean()
        S[i] = S[i - 1] - s_i + i_s
        I[i] = I[i - 1] + s_i - i_s
...
# tested
def seir(beta: float, mu_ei: float, mu_ir: float, S0: int, E0: int, I0: int, R0: int, time: int, x_label: str, plot = True):
    """
    ...
    for i in range(1, time + 1):
        p = beta * I[i - 1] / n
        s_e = np.random.binomial(S[i - 1], p, 1000).mean()
        e_i = np.random.binomial(E[i - 1], mu_ei, 1000).mean()
        i_r = np.random.binomial(I[i - 1], mu_ir, 1000).mean()
        S[i] = S[i - 1] - s_e
        E[i] = E[i - 1] + s_e - e_i
        I[i] = I[i - 1] + e_i - i_r
        R[i] = R[i - 1] + i_r
...
def sirs(beta: float, mu_ir: float, mu_rs: float, S0: int, I0: int, R0: int, time:int, xlabel = "Hours", title = "SIRS", fname = "SIRSfig.png",plot = True):
    def __init_sirs__(S0: int, I0: int, R0: int, time: int):
        S, I, R, t = np.zeros(time+1), np.zeros(time+1), np.zeros(time+1), np.zeros(time+1)
        S[0], I[0], R[0] = S0, I0, R0
        for i in range(t+1):
            t[i] = i
        return S, I, R, t
    n = S0 + I0 + R0
    S, I, R, t = __init_sirs__(S0, I0, R0, time)
    for i in range(1, time+1):
        p = beta * I[i-1] / n
        s_i = np.random.binomial(S[i-1], p, 1000).mean()
        i_r = np.random.binomial(I[i-1], mu_ir, 1000).mean()
        r_s = np.random.binomial(R[i-1], mu_rs, 1000).mean()
        S[i] = S[i-1] - s_i + r_s
        I[i] = I[i-1] + s_i - i_r
        R[i] = R[i-1] + i_r - r_s
    if plot == True:
