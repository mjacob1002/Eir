...
mu: the probability that an individual goes from I compartment to D compartment, given that the person didn't go from I to R in the same timestep. Must be a value that belongs to [0,1]. Throws ```python ProbabilityException``` or ```python NotFloatException``` otherwise. 